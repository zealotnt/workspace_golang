...
func main() {
	...
	if test_mode == 2 {
		...
		mode_enc := cipher.NewCBCEncrypter(block, iv)
		fmt.Printf("InitVector:\t% x\n", iv)
		mode_enc.CryptBlocks(ciphertext, plaintext)
		...
		mode_dec := cipher.NewCBCDecrypter(block, iv)
		mode_dec.CryptBlocks(decryptedtext, ciphertext)
